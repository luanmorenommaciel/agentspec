...
When the spec enumerates its triggers, the declared count must agree with the
enumeration — a count that outruns the scenarios behind it is exactly the claim
the overlap and trigger criteria exist to test. Every threshold is a constructor
parameter, so revising a criterion changes call sites rather than code.
"""
...
class CreationSpecContract:
    ...
    def check(self, parsed: dict[str, Any]) -> list[Finding]:
        return [
            ...
            *self._scenario_findings(parsed),
    ...
    def _scenario_findings(self, spec: dict[str, Any]) -> list[Finding]:
        """`trigger_scenarios` is optional, but a spec that enumerates its triggers
        must not also claim a different number of them. The count is only compared
        once it is itself well-formed, so a bad count is reported by one rule, not
        two."""
        scenarios = spec.get("trigger_scenarios")
        if scenarios is None:
            return []
        count = spec.get("trigger_count")
        ...
        if not isinstance(scenarios, (list, tuple)):
            return [
                _fail(
                    ...
                    "trigger_scenarios",
                    "trigger_scenarios must be a list of the scenarios trigger_count counts",
                    f"a list of {count} scenarios",
                    repr(scenarios),
                )
            ]
        if len(scenarios) != count:
            return [
                _fail(
                    "L2.trigger_count_mismatch",
                    "trigger_scenarios",
                    "trigger_count disagrees with the scenarios actually enumerated",
                    f"{count} scenarios",
