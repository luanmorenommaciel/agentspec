...
```python
from spec_composer import compose          # compose(request, pipeline, *, resolver=None, generator=None, evaluator=None) -> ComposeResult
...
Three seams are injected into `compose()`, all keyword-only and all defaulting to a working binding: `compose(request, pipeline, *, resolver=None, generator=None, evaluator=None)`. The conductor holds no model vocabulary and no generation logic of its own — it only dispatches to whichever concrete object each seam is given.

**`ContractResolver`.** A stage's `input_contract` / `output_contract` is a name, not an object. A resolver binds that name to a `spec_linter.Contract` via `resolve(name) -> Contract`, raising `UnresolvedContract` when nothing is bound. `DefaultResolver` — used when no resolver is passed — binds `creation-spec` to `CreationSpecContract()` and `agent-spec` to `AgentSpecContract()`, both from the sibling `spec_linter`. An unresolvable name is a configuration defect, not a transient failure: it exits 2 (§5), because re-running cannot clear it.
...
| `no-feedback-target` | blocked | A gate FAIL with no producing stage before it to route to |
| `high-assurance-judge-fail` | blocked | A blocking behavioral verdict at the `high-assurance` tier, pending human review |
| `judge-unparseable` | error | The judge stage's subject could not be structurally parsed — the only way a judge FAIL can occur below the `high-assurance` tier, since `smoke`/`standard` are WARN-capped by construction |
...
Every run **folds** `run.jsonl` from scratch before it does anything else — there is no in-memory state carried between invocations. The fold produces the current epoch (one plus the number of `run-closed` rows), `attempts_spent` (the number of `spend` rows since the last `run-closed`), the granted approvals, and every stamp. A stage is skipped when a non-FAIL stamp already certifies its current subject: a `lint` stage compares its input against the stamp, a producing (`create`/`generate`) stage compares both its input and its output, and an `emit` stage compares only the promoted target's content — so re-running the identical command after an interruption simply resumes from the first stage whose evidence no longer matches what is on disk now.
