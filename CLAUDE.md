...
**What is AgentSpec?** A Claude Code plugin that provides structured AI-assisted development through a 5-phase SDD workflow, specialized for data engineering with 58 agents, 31 commands, 24 KB domains, and 12 skills (9 distributed in the plugin + 3 repo-local).
...
│   │
│   ├── skills/              # Reusable capability packs (10 source skills)
...
│   │   ├── github-cr-adr/   # ADR drafting (worthiness gate, dedup, ephemeral drafts)
│   │   ├── github-cr-issue/ # Typed issue drafting (feature/component/task/bug/spike)
│   │   ├── github-post-issue/ # Guarded gh publishing + board curation
│   │   ├── skill-create/    # Repo conventions for new skills; defers to upstream skill-creator (repo-local)
│   │   ├── kb-build/        # High-assurance, source-verified KB building
│   │   ├── meeting-analysis/ # Transcript → analysis doc + channel follow-up (repo-local)
│   │   └── standup-report/  # Daily Done / Will do / Blockers message (repo-local)
│   │
│   ├── sdd/                 # SDD framework
...
│   ├── skills/              # 9 skills (7 from .claude/ + 2 plugin-only; repo-local excluded)
...
| `.claude/skills/skill-create/` · `kb-build/` | Authoring SOPs: new skills (repo-local; defers to upstream `skill-creator`), high-assurance KB domains |
